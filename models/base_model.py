...
class BaseModel(ABC):
    ...
    def get_current_losses_eval(self):
        """Return traning losses / errors. train.py will print out these errors on console, and save them to a file"""
        errors_ret = OrderedDict()
        for name in self.loss_names_eval:
            if isinstance(name, str):
                errors = getattr(self, 'loss_eval_' + name)
                if isinstance(errors, tuple):
                    errors_ret[name] = errors  # float(...) works for both scalar tensor and float number
                else:
                    errors_ret[name] = float(errors)
        return errors_ret
